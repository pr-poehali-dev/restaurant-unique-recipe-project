    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="pt-32 pb-20 px-6">
        <div className="container mx-auto text-center max-w-4xl">
          <h2 className="text-6xl md:text-7xl font-bold mb-6 animate-fade-in">
            Блюдо, созданное для вас
          </h2>
          <p className="text-xl text-gray-600 mb-8 animate-fade-in">
            Заполните анкету, чтобы получить уникальную идею для блюда, рецепт и список покупок, идеально соответствующие вашему запросу
          </p>
      ...
      <section id="questionnaire" className="py-20 px-6 bg-gray-50">
        <div className="container mx-auto max-w-3xl">
          ...
          <p className="text-center text-gray-600 mb-12">Шаг {currentSection + 1} из {sections.length}</p>

          <Card className="mb-8">
            <CardContent className="p-8">
              <h3 className="text-3xl font-bold mb-8">{sections[currentSection].title}</h3>
                      ...
                      <RadioGroup value={formData[field.name as keyof typeof formData]} onValueChange={(value) => handleChange(field.name, value)}>
                        <div className="space-y-3">
                          {field.options.map((option) => (
                            <div key={option} className="flex items-center space-x-3 p-3 hover:bg-gray-50 rounded-lg transition">
                              <RadioGroupItem value={option} id={`${field.name}-${option}`} />
                              <Label htmlFor={`${field.name}-${option}`} className="flex-1 cursor-pointer">{option}</Label>
                            </div>
                          ))}
                        </div>
                      </RadioGroup>
            ...
            {sections.map((_, index) => (
              <div 
                ...
                className={`h-2 rounded-full transition-all ${
                  index === currentSection ? 'w-8 bg-primary' : 'w-2 bg-gray-300'
                }`}
              />
            ))}
          </div>
